      ...
      >
        <header className="flex items-start justify-between gap-4 border-b border-border px-4 py-4 sm:px-6">
          <div className="min-w-0">
            <h2 className="text-lg font-semibold leading-6 text-foreground">Parking Charges</h2>
        ...
        <div className="max-h-[calc(88vh-76px)] overflow-y-auto">
          <section className="space-y-3 px-4 py-4 sm:px-6">
            ...
            <div className="grid grid-cols-[minmax(0,1fr)_74px_86px] gap-2 px-1 text-[11px] font-medium uppercase tracking-normal text-muted-foreground">
              <span>Parking</span>
              <span>Status</span>
              ...
              <div className="space-y-2 pb-2">
                {filtered.map((transaction) => {
                  ...
                  return (
                    <div
                      key={transaction.id}
                      className="grid min-h-11 grid-cols-[minmax(0,1fr)_74px_86px] items-center gap-2 rounded-md border border-border bg-muted/10 px-3 py-3"
                    >
                      <div className="min-w-0">
                        <p className="truncate text-sm font-medium text-foreground">{transaction.location || "Parking"}</p>
                        <p className="mt-1 truncate font-ibm-plex-mono text-[11px] tabular-nums text-muted-foreground">
                          {formatDateTime(transaction.parking_date)}{transaction.parking_zone ? ` · Zone ${transaction.parking_zone}` : ""}
                        </p>
                        <p className="mt-1 truncate text-[11px] text-muted-foreground">{vehicle || "No car"}</p>
                      </div>
                      <span className={`inline-flex w-fit items-center rounded-full border px-2 py-0.5 text-[11px] font-semibold ${isPaid ? "border-tint-green-foreground/20 bg-tint-green text-tint-green-foreground" : "border-tint-rose-foreground/20 bg-tint-rose text-tint-rose-foreground"}`}>
                        {isPaid ? "Paid" : transaction.status || "Unpaid"}
                      </span>
                      <p className="text-right font-ibm-plex-mono text-xs font-semibold tabular-nums text-foreground">
