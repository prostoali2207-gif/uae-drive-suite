...
import { CheckCircle2, ChevronDown, FileText, Loader2, PenLine, Trash2 } from "lucide-react";
...
function SignaturePad({ onSave }: { onSave: (value: string) => Promise<void> }) {
  ...
  useEffect(clear, []);
  const point = (event: React.PointerEvent<HTMLCanvasElement>) => { const rect = event.currentTarget.getBoundingClientRect(); return { x: (event.clientX - rect.left) * 900 / rect.width, y: (event.clientY - rect.top) * 420 / rect.height }; };
  ...
  const move = (event: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    ctx.strokeStyle = "#111827"; ctx.lineWidth = Math.max(2.5, Math.min(6, 7 - distance * .12)); ctx.lineCap = "round"; ctx.lineJoin = "round";
    ctx.beginPath(); ctx.moveTo(last.current.x, last.current.y); ctx.lineTo(next.x, next.y); ctx.stroke(); last.current = next; setHasInk(true);
  };
  ...
  return <div className="mt-4"><canvas ref={ref} width={900} height={420} className="h-[42dvh] min-h-64 w-full touch-none rounded-xl border-2 border-cyan-400 bg-white" onPointerDown={start} onPointerMove={move} onPointerUp={stop} onPointerCancel={stop} /><div className="mt-3 grid grid-cols-2 gap-3"><Button variant="outline" className="h-12" onClick={clear}><Trash2 className="mr-2 h-4 w-4" />Clear</Button><Button className="h-12 bg-cyan-600 text-white" disabled={!hasInk || saving} onClick={submit}>{saving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Done</Button></div></div>;
}
...
export default function PublicContractSign() {
  ...
  const [termsOpen, setTermsOpen] = useState(false), [accepted, setAccepted] = useState(false);
  ...
  return <div className="min-h-screen bg-slate-100 text-slate-950">
    ...
    <main className="mx-auto max-w-3xl space-y-4 p-3 pb-10 sm:p-6">
      ...
      <section className="rounded-xl border bg-white p-4"><h2 className="font-bold">Terms</h2><div className="mt-3 space-y-2 text-sm">{(keyTerms.length ? keyTerms : ["The customer accepts the rental terms and vehicle responsibility."]).map((item, i) => <div key={i}>• {item}</div>)}</div><button className="mt-3 flex min-h-11 w-full items-center justify-between rounded-lg border px-3 text-sm font-semibold" onClick={() => setTermsOpen(!termsOpen)}>Full contract terms<ChevronDown className={`h-4 w-4 ${termsOpen ? "rotate-180" : ""}`} /></button>{termsOpen && <div className="mt-3 whitespace-pre-wrap rounded-lg bg-slate-50 p-3 text-xs leading-relaxed">{data.company.terms || "No additional terms."}</div>}<label className="mt-4 flex min-h-12 gap-3 rounded-lg border-2 border-cyan-300 bg-cyan-50 p-3 text-sm"><input type="checkbox" className="h-5 w-5" checked={accepted} disabled={data.accepted} onChange={(e) => setAccepted(e.target.checked)} /><span>I have read and accept the contract terms.</span></label></section>
    ...
    {active && <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-100 p-3"><div className="mx-auto max-w-4xl"><div className="flex items-center justify-between"><div><div className="text-xs font-bold uppercase text-cyan-700">Signature</div><div className="text-lg font-bold">{active.name}</div></div><Button variant="outline" onClick={() => setActive(null)}>Cancel</Button></div><SignaturePad onSave={save} /></div></div>}
  </div>;
}
