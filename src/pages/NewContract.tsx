    ...
    <DashboardLayout title="New Contract" subtitle="Create, inspect and sign" mobileContractsNav>
      <div className="-mx-4 -my-6 min-h-[calc(100dvh-3.5rem)] bg-[#f5f7f9] text-slate-950 md:-mx-8 md:-my-8">
        ...
        <div className="mx-auto max-w-5xl px-3 py-4 md:px-8 md:py-8">
          ...
          {step === 3 && <div className="space-y-4"><div><h2 className="text-2xl font-bold">Review & sign</h2><p className="mt-1 text-sm text-slate-600">The booking becomes signed after every required signature is saved. Start the rental only when the vehicle is handed over.</p></div><section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:p-5"><div className="grid grid-cols-2 gap-2.5 sm:gap-3"><Summary label="Client" value={selectedClient?.full_name || "—"} wide /><Summary label="Vehicle" value={selectedCar ? `${selectedCar.plate} — ${selectedCar.make} ${selectedCar.model}` : "—"} wide /><Summary label="Rental period" value={`${form.start_date} ${form.start_time} → ${form.end_date} ${form.end_time}`} wide /><Summary label="Rate" value={`${form.rate_type} · AED ${Number(form.rate_amount || 0).toLocaleString()}`} /><Summary label="Initial mileage" value={`${Number(form.initial_mileage || 0).toLocaleString()} ${form.mileage_unit}`} /><Summary label="Fuel" value={form.fuel_level} /><Summary label="Rental total" value={`AED ${rentalTotal.toLocaleString()}`} /><Summary label="Additional charges" value={`AED ${extrasTotal.toLocaleString()}`} /><Summary label="Deposit held separately" value={`AED ${Number(form.deposit_amount || 0).toLocaleString()}`} accent="amber" /><Summary label="Pickup photos" value={`${photos.length} photo${photos.length === 1 ? "" : "s"}`} /></div><div className="mt-3 rounded-xl bg-cyan-50 p-3.5"><div className="text-xs font-semibold text-cyan-800">Contract total</div><div className="font-mono text-2xl font-bold text-cyan-950">AED {grandTotal.toLocaleString()}</div></div></section><div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-between"><Button variant="outline" className={cn("h-12", lightOutlineClass)} onClick={() => setStep(2)}>Back to photos</Button><Button className="h-12 bg-cyan-600 px-7 text-white" onClick={() => setSignOpen(true)}>Open agreement & sign</Button></div><SignContractModal contractId={contractId} clientName={clientName} open={signOpen} onComplete={finishSignedContract} /></div>}

          {step === 4 && <div className="mx-auto max-w-2xl py-12 text-center"><div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-emerald-100 text-emerald-700"><Check className="h-10 w-10" /></div><h2 className="mt-6 text-3xl font-bold">Contract signed</h2><p className="mt-2 text-slate-600">The booking is signed. Start the rental from Contract Details when the vehicle is handed over.</p><div className="mt-8 grid gap-3 sm:grid-cols-2"><Button variant="outline" className={cn("h-12", lightOutlineClass)} onClick={() => navigate(`/contracts/${contractId}`)}>Open contract</Button><Button className="h-12 bg-cyan-600 text-white" onClick={() => navigate("/contracts")}>Back to contracts</Button></div></div>}
        </div>
      </div>
    </DashboardLayout>
  );
