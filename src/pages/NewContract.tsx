...
export default function NewContract() {
  ...
  const clientWarning = useMemo(() => {
    ...
    if (!selectedClient.license_number?.trim() || !selectedClient.license_expiry) return "Driving licence information is incomplete.";
  ...
  const createContract = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Could not create contract draft.");
  ...
  const activateSignedContract = async () => {
    ...
    if (error) { toast.error("Signatures were saved, but the contract could not be activated."); return; }
    ...
    <DashboardLayout title="New Contract" subtitle="Create, inspect and sign" mobileContractsNav>
      <div className="-mx-4 -my-6 min-h-[calc(100dvh-3.5rem)] bg-[#f5f7f9] text-slate-950 md:-mx-8 md:-my-8">
        ...
        <div className="mx-auto max-w-5xl px-3 py-4 md:px-8 md:py-8">
            ...
            <div className="space-y-4">
              ...
              <section className="rounded-xl border border-slate-200 bg-white p-4 md:p-5">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div ref={(node) => { fieldRefs.current.client_id = node; }} className="grid gap-2"><Label>Main client *</Label><Popover open={clientOpen} onOpenChange={setClientOpen}><PopoverTrigger asChild><Button variant="outline" className={cn("h-11 justify-between !border-2 !bg-white !text-slate-950", fieldErrors.client_id ? "!border-red-500" : "!border-cyan-500")}>{selectedClient?.full_name || "Select client"}<ChevronRight className="h-4 w-4 text-slate-400" /></Button></PopoverTrigger><PopoverContent className="z-[100] w-[var(--radix-popover-trigger-width)] border-2 border-cyan-500 bg-white p-0"><Command><CommandInput placeholder="Search client" value={clientSearch} onValueChange={setClientSearch} /><CommandList><CommandEmpty>No client found.</CommandEmpty><CommandGroup>{filteredClients.map((client) => <CommandItem key={client.id} value={client.id} onSelect={() => { setField("client_id", client.id); setAdditionalDriverIds((ids) => ids.filter((id) => id !== client.id)); setClientOpen(false); }}><Check className={cn("mr-2 h-4 w-4", form.client_id === client.id ? "opacity-100" : "opacity-0")} />{client.full_name}</CommandItem>)}</CommandGroup></CommandList></Command></PopoverContent></Popover><FieldError field="client_id" />{clientWarning && <div className="flex gap-2 rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" /><span>{clientWarning}</span></div>}</div>
                  <div ref={(node) => { fieldRefs.current.car_id = node; }} className="grid gap-2"><Label>Vehicle *</Label><Popover open={carOpen} onOpenChange={setCarOpen}><PopoverTrigger asChild><Button variant="outline" className={cn("h-11 justify-between !border-2 !bg-white !text-slate-950", fieldErrors.car_id ? "!border-red-500" : "!border-cyan-500")}>{selectedCar ? `${selectedCar.plate} — ${selectedCar.make} ${selectedCar.model}` : "Select available vehicle"}<ChevronRight className="h-4 w-4 text-slate-400" /></Button></PopoverTrigger><PopoverContent className="z-[100] w-[var(--radix-popover-trigger-width)] border-2 border-cyan-500 bg-white p-0"><Command><CommandInput placeholder="Search plate or model" value={carSearch} onValueChange={setCarSearch} /><CommandList><CommandEmpty>No available vehicle found.</CommandEmpty><CommandGroup>{filteredCars.map((car) => <CommandItem key={car.id} value={car.id} onSelect={() => { prefillMileage(car); setCarOpen(false); }}><Check className={cn("mr-2 h-4 w-4", form.car_id === car.id ? "opacity-100" : "opacity-0")} />{car.plate} — {car.make} {car.model}</CommandItem>)}</CommandGroup></CommandList></Command></PopoverContent></Popover><FieldError field="car_id" /></div>
                </div>
              ...
              <section className="rounded-xl border border-slate-200 bg-white p-4 md:p-5">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div ref={(node) => { fieldRefs.current.initial_mileage = node; }} className="grid gap-2"><div className="flex items-center justify-between gap-3"><Label>Initial mileage *</Label><div className="grid grid-cols-2 rounded-lg bg-slate-100 p-1">{(["km", "mi"] as MileageUnit[]).map((unit) => <Button key={unit} type="button" variant="ghost" className={cn("h-8 px-3 text-xs uppercase", form.mileage_unit === unit && "bg-white text-cyan-700 shadow-sm")} disabled={!form.car_id} onClick={() => setField("mileage_unit", unit)}>{unit}</Button>)}</div></div><Input type="number" min={0} className={cn(inputClass, fieldErrors.initial_mileage && errorInputClass)} value={form.initial_mileage} onChange={(e) => setField("initial_mileage", e.target.value)} /><FieldError field="initial_mileage" />{lastMileage != null && <p className="text-xs text-slate-600">Last recorded: {lastMileage.toLocaleString()} {form.mileage_unit}</p>}{lastMileage != null && Number(form.initial_mileage) < lastMileage && <p className="flex items-center gap-1 text-xs font-medium text-amber-700"><AlertTriangle className="h-3.5 w-3.5" />Entered mileage is below the last recorded value.</p>}</div>
                ...
                <div className="mt-5 space-y-2">{charges.map((charge) => <div key={charge.id} className="grid grid-cols-[1fr_120px_44px] gap-2"><Select value={charge.label} onValueChange={(value) => setCharges((all) => all.map((item) => item.id === charge.id ? { ...item, label: value as ChargeLabel } : item))}><SelectTrigger className={inputClass}><SelectValue /></SelectTrigger><SelectContent>{chargeLabels.map((label) => <SelectItem key={label} value={label}>{label}</SelectItem>)}</SelectContent></Select><Input type="number" min={0} step="0.01" placeholder="AED" className={inputClass} value={charge.amount} onChange={(e) => setCharges((all) => all.map((item) => item.id === charge.id ? { ...item, amount: e.target.value } : item))} /><Button type="button" variant="outline" className="h-11" onClick={() => setCharges((all) => all.filter((item) => item.id !== charge.id))}><X className="h-4 w-4" /></Button></div>)}<Button type="button" variant="outline" className="h-11 border-cyan-300 bg-cyan-50 text-cyan-700" onClick={() => setCharges((all) => [...all, { id: uuid(), label: "Delivery", amount: "" }])}><Plus className="mr-2 h-4 w-4" /> Add charge</Button></div>
                <div className="mt-5 grid gap-4 md:grid-cols-2"><div ref={(node) => { fieldRefs.current.deposit_amount = node; }} className="grid gap-2 rounded-xl border-2 border-amber-300 bg-amber-50 p-4"><Label className="font-bold text-amber-950">Security deposit (AED)</Label><Input type="number" min={0} className={cn(inputClass, fieldErrors.deposit_amount && errorInputClass)} value={form.deposit_amount} onChange={(e) => setField("deposit_amount", e.target.value)} /><FieldError field="deposit_amount" /><p className="text-xs text-amber-800">Deposit is separate from rental payments.</p></div><div className="grid gap-2"><Label>Notes</Label><Textarea className="min-h-28" placeholder="Optional internal notes" value={form.notes} onChange={(e) => setField("notes", e.target.value)} /></div></div>
              </section>
          ...
          {step === 2 && <div className="space-y-4"><div><h2 className="text-2xl font-bold">Vehicle condition</h2><p className="mt-1 text-sm text-slate-600">Add pickup photos before the client signs. Maximum 10.</p></div><section className="rounded-xl border border-slate-200 bg-white p-4 md:p-5"><div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-5">{photos.map((photo) => <div key={photo.id} className="relative overflow-hidden rounded-xl border border-slate-200 bg-slate-100"><img src={photoPreviews[photo.id]} alt="Pickup inspection" className="aspect-square w-full object-cover" /><button type="button" aria-label="Delete photo" onClick={() => deletePhoto(photo)} className="absolute right-2 top-2 flex h-9 w-9 items-center justify-center rounded-full bg-white text-red-600 shadow"><X className="h-4 w-4" /></button></div>)}</div>{photos.length === 0 && <div className="flex min-h-52 flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 text-center"><ImageIcon className="h-9 w-9 text-cyan-600" /><p className="mt-3 font-semibold">No pickup photos yet</p><p className="text-sm text-slate-500">Take clear photos of all sides and existing damage.</p></div>}<input ref={cameraRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={(e) => { uploadPhotos(e.target.files); e.target.value = ""; }} /><input ref={galleryRef} type="file" accept="image/*" multiple className="hidden" onChange={(e) => { uploadPhotos(e.target.files); e.target.value = ""; }} /><div className="mt-5 grid gap-3 sm:grid-cols-2"><Button variant="outline" className="h-12 border-cyan-300 bg-cyan-50 text-cyan-700" disabled={uploading || photos.length >= 10} onClick={() => cameraRef.current?.click()}>{uploading ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <Camera className="mr-2 h-5 w-5" />}Take photo</Button><Button variant="outline" className="h-12" disabled={uploading || photos.length >= 10} onClick={() => galleryRef.current?.click()}><ImageIcon className="mr-2 h-5 w-5" />Choose from gallery</Button></div></section><div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><Button variant="outline" className="h-12" onClick={() => setStep(3)}>Skip photos for now</Button><Button className="h-12 bg-cyan-600 px-7 text-white" onClick={() => setStep(3)}>Continue to review</Button></div></div>}

          {step === 3 && <div className="space-y-4"><div><h2 className="text-2xl font-bold">Review & sign</h2><p className="mt-1 text-sm text-slate-600">The draft becomes active after every required signature is saved.</p></div><section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="grid gap-4 sm:grid-cols-2"><Summary label="Client" value={selectedClient?.full_name || "—"} /><Summary label="Vehicle" value={selectedCar ? `${selectedCar.plate} — ${selectedCar.make} ${selectedCar.model}` : "—"} /><Summary label="Rental period" value={`${form.start_date} ${form.start_time} → ${form.end_date} ${form.end_time}`} /><Summary label="Rate" value={`${form.rate_type} · AED ${Number(form.rate_amount || 0).toLocaleString()}`} /><Summary label="Initial mileage" value={`${Number(form.initial_mileage || 0).toLocaleString()} ${form.mileage_unit}`} /><Summary label="Fuel" value={form.fuel_level} /><Summary label="Rental total" value={`AED ${rentalTotal.toLocaleString()}`} /><Summary label="Additional charges" value={`AED ${extrasTotal.toLocaleString()}`} /><Summary label="Deposit held separately" value={`AED ${Number(form.deposit_amount || 0).toLocaleString()}`} accent="amber" /><Summary label="Pickup photos" value={`${photos.length} photo${photos.length === 1 ? "" : "s"}`} /></div><div className="mt-5 rounded-xl bg-cyan-50 p-4"><div className="text-sm font-semibold text-cyan-800">Contract total</div><div className="font-mono text-3xl font-bold text-cyan-950">AED {grandTotal.toLocaleString()}</div></div></section><div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-between"><Button variant="outline" className="h-12" onClick={() => setStep(2)}>Back to photos</Button><Button className="h-12 bg-cyan-600 px-7 text-white" onClick={() => setSignOpen(true)}>Open agreement & sign</Button></div><SignContractModal contractId={contractId} clientName={clientName} open={signOpen} onComplete={activateSignedContract} /></div>}

          {step === 4 && <div className="mx-auto max-w-2xl py-12 text-center"><div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-emerald-100 text-emerald-700"><Check className="h-10 w-10" /></div><h2 className="mt-6 text-3xl font-bold">Contract signed</h2><p className="mt-2 text-slate-600">The contract is active and the vehicle status has been updated.</p><div className="mt-8 grid gap-3 sm:grid-cols-2"><Button variant="outline" className="h-12" onClick={() => navigate(`/contracts/${contractId}`)}>Open contract</Button><Button className="h-12 bg-cyan-600 text-white" onClick={() => navigate("/contracts")}>Back to contracts</Button></div></div>}
        </div>
      </div>
    </DashboardLayout>
  );
