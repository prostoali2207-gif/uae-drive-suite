...
const inputClass = "h-11 !border !border-input !bg-input !text-foreground placeholder:!text-muted-foreground shadow-none focus-visible:!border-primary focus-visible:!ring-2 focus-visible:!ring-ring";
const lightOutlineClass = "border-border !bg-card !text-foreground hover:!bg-muted/40 hover:!text-foreground";
const errorInputClass = "!border-destructive focus-visible:!border-destructive focus-visible:!ring-destructive/20";
...
export default function NewContract() {
  ...
  const FieldError = ({ field }: { field: FieldKey }) => fieldErrors[field] ? <p className="text-xs font-medium text-destructive">{fieldErrors[field]}</p> : null;
  ...
  return (
    <DashboardLayout title="New Contract" subtitle="Create, inspect and sign" mobileContractsNav>
      <div className="-mx-4 -my-6 min-h-[calc(100dvh-3.5rem)] bg-background text-foreground md:-mx-8 md:-my-8">
        <div className="sticky top-0 z-20 border-b border-primary/40 bg-primary text-primary-foreground shadow-sm">
          <div className="mx-auto max-w-5xl px-4 py-4 md:px-8">
            <div className="flex items-center justify-between gap-3">
              <Button variant="ghost" className="h-11 px-2 text-primary-foreground hover:bg-primary-foreground/10 hover:text-primary-foreground" onClick={() => navigate("/contracts")}><ArrowLeft className="mr-2 h-4 w-4" /> Contracts</Button>
              <span className="text-sm font-bold text-primary-foreground">{step}/4</span>
            </div>
            <div className="mt-3 grid grid-cols-4 gap-2">{stepLabels.map((label, index) => <div key={label}><div className={cn("h-2 rounded-full", index + 1 <= step ? "bg-primary-foreground" : "bg-primary-foreground/35")} /><div className={cn("mt-1 hidden text-[11px] font-semibold md:block", index + 1 === step ? "text-primary-foreground" : "text-primary-foreground/75")}>{label}</div></div>)}</div>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-5xl px-3 py-4 md:px-8 md:py-8">
          {step === 1 && (
            <div className="space-y-4">
              <div><h2 className="text-2xl font-bold">Contract details</h2><p className="mt-1 text-sm text-muted-foreground">Complete the rental details. The contract becomes active only after all signatures.</p></div>

              <section className="rounded-xl border border-border bg-card p-4 md:p-5">
                <h3 className="mb-4 text-sm font-bold uppercase tracking-wide text-primary">Client & vehicle</h3>
                <div className="grid gap-4 md:grid-cols-2">
                  <div ref={(node) => { fieldRefs.current.client_id = node; }} className="grid gap-2"><Label>Main client *</Label><Popover open={clientOpen} onOpenChange={(open) => { setClientOpen(open); if (!open) setClientSearch(""); }}><PopoverTrigger asChild><Button variant="outline" className={cn("h-11 justify-between !border !bg-input !text-foreground", fieldErrors.client_id ? "!border-destructive" : "!border-input")}>{selectedClient?.full_name || "Select client"}<ChevronRight className="h-4 w-4 text-muted-foreground" /></Button></PopoverTrigger><PopoverContent className="z-[100] w-[var(--radix-popover-trigger-width)] !border !border-border !bg-popover p-0 !text-popover-foreground"><Command shouldFilter={false} className="!bg-popover !text-popover-foreground"><CommandInput className="!text-foreground placeholder:!text-muted-foreground" placeholder="Search client" value={clientSearch} onValueChange={setClientSearch} /><CommandList className="!bg-popover"><CommandEmpty className="!text-muted-foreground">No client found.</CommandEmpty><CommandGroup className="!text-foreground">{filteredClients.map((client) => <CommandItem className="!text-foreground data-[selected=true]:!bg-primary/10 data-[selected=true]:!text-foreground" key={client.id} value={`${client.full_name} ${client.license_number || ""}`} onSelect={() => { setField("client_id", client.id); setAdditionalDriverIds((ids) => ids.filter((id) => id !== client.id)); setClientOpen(false); setClientSearch(""); }}><Check className={cn("mr-2 h-4 w-4", form.client_id === client.id ? "opacity-100" : "opacity-0")} />{client.full_name}</CommandItem>)}</CommandGroup></CommandList></Command></PopoverContent></Popover><FieldError field="client_id" />{clientWarning && <div className="flex gap-2 rounded-lg border border-tint-amber-foreground/20 bg-tint-amber p-3 text-sm text-tint-amber-foreground"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" /><span>{clientWarning}</span></div>}</div>
                  <div ref={(node) => { fieldRefs.current.car_id = node; }} className="grid gap-2"><Label>Vehicle *</Label><Popover open={carOpen} onOpenChange={(open) => { setCarOpen(open); if (!open) setCarSearch(""); }}><PopoverTrigger asChild><Button variant="outline" className={cn("h-11 justify-between !border !bg-input !text-foreground", fieldErrors.car_id ? "!border-destructive" : "!border-input")}>{selectedCar ? `${selectedCar.plate} — ${selectedCar.make} ${selectedCar.model}` : "Select available vehicle"}<ChevronRight className="h-4 w-4 text-muted-foreground" /></Button></PopoverTrigger><PopoverContent className="z-[100] w-[var(--radix-popover-trigger-width)] !border !border-border !bg-popover p-0 !text-popover-foreground"><Command shouldFilter={false} className="!bg-popover !text-popover-foreground"><CommandInput className="!text-foreground placeholder:!text-muted-foreground" placeholder="Search plate or model" value={carSearch} onValueChange={setCarSearch} /><CommandList className="!bg-popover"><CommandEmpty className="!text-muted-foreground">No available vehicle found.</CommandEmpty><CommandGroup className="!text-foreground">{filteredCars.map((car) => <CommandItem className="!text-foreground data-[selected=true]:!bg-primary/10 data-[selected=true]:!text-foreground" key={car.id} value={`${car.plate} ${car.make} ${car.model}`} onSelect={() => { prefillMileage(car); setCarOpen(false); setCarSearch(""); }}><Check className={cn("mr-2 h-4 w-4", form.car_id === car.id ? "opacity-100" : "opacity-0")} />{car.plate} — {car.make} {car.model}</CommandItem>)}</CommandGroup></CommandList></Command></PopoverContent></Popover><FieldError field="car_id" /></div>
                </div>
              ...
              <section className="rounded-xl border border-border bg-card p-4 md:p-5">
                <h3 className="mb-4 text-sm font-bold uppercase tracking-wide text-primary">Rental period & rate</h3>
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-3 max-[340px]:gap-2"><div className="grid min-w-0 gap-2"><Label>Start date *</Label><Input type="date" className={cn(inputClass, "min-w-0 max-[340px]:px-2 max-[340px]:text-xs")} value={form.start_date} onChange={(e) => setField("start_date", e.target.value)} /></div><div ref={(node) => { fieldRefs.current.end_date = node; }} className="grid min-w-0 gap-2"><Label>End date *</Label><Input type="date" className={cn(inputClass, "min-w-0 max-[340px]:px-2 max-[340px]:text-xs", fieldErrors.end_date && errorInputClass)} value={form.end_date} onChange={(e) => setField("end_date", e.target.value)} /><FieldError field="end_date" /></div></div>
                  <div className="grid grid-cols-2 gap-3 max-[340px]:gap-2"><div className="grid min-w-0 gap-2"><Label>Start time *</Label><Input type="time" className={cn(inputClass, "min-w-0 max-[340px]:px-2 max-[340px]:text-xs")} value={form.start_time} onChange={(e) => setField("start_time", e.target.value)} /></div><div ref={(node) => { fieldRefs.current.end_time = node; }} className="grid min-w-0 gap-2"><Label>End time *</Label><Input type="time" className={cn(inputClass, "min-w-0 max-[340px]:px-2 max-[340px]:text-xs", fieldErrors.end_time && errorInputClass)} value={form.end_time} onChange={(e) => setField("end_time", e.target.value)} /><FieldError field="end_time" /></div></div>
                </div>
                <div className="mt-4 grid gap-4 md:grid-cols-2"><div><Label className="mb-2 block">Rate type</Label><div className="grid grid-cols-3 gap-2 rounded-xl bg-muted p-1">{rateTypes.map((type) => <Button key={type} type="button" variant="ghost" className={cn("h-10 text-muted-foreground", form.rate_type === type && "bg-primary/10 text-primary shadow-sm")} onClick={() => setField("rate_type", type)}>{type}</Button>)}</div></div><div ref={(node) => { fieldRefs.current.rate_amount = node; }} className="grid gap-2"><Label>{form.rate_type} rate (AED) *</Label><Input type="number" min={0} className={cn(inputClass, fieldErrors.rate_amount && errorInputClass)} value={form.rate_amount} onChange={(e) => setField("rate_amount", e.target.value)} /><FieldError field="rate_amount" /></div></div>
                <p className="mt-3 text-sm font-medium text-muted-foreground">{form.rate_type === "Monthly" && form.end_date ? formatMonthlyBillingPeriod(form.start_date, form.end_date) : `${rentalDays} rental day${rentalDays === 1 ? "" : "s"}`}</p>
              </section>

              <section className="rounded-xl border border-border bg-card p-4 md:p-5">
                <h3 className="mb-4 text-sm font-bold uppercase tracking-wide text-primary">Handover, extras & deposit</h3>
                <div className="grid gap-4 md:grid-cols-2">
                  <div ref={(node) => { fieldRefs.current.initial_mileage = node; }} className="grid gap-2">
                    <div className="flex items-center justify-between gap-3"><Label>Initial mileage *</Label><div className="flex items-center gap-2"><span className="rounded-md border border-primary/20 bg-primary/10 px-2.5 py-1 text-xs font-bold uppercase text-primary">{form.mileage_unit}</span><button type="button" disabled={!form.car_id} onClick={changeMileageUnit} className="text-xs font-semibold text-primary underline-offset-2 hover:underline disabled:text-muted-foreground">Change</button></div></div>
                    ...
                    {lastMileage != null && <p className="text-xs text-muted-foreground">Last recorded: {lastMileage.toLocaleString()} {form.mileage_unit}</p>}
                    {lastMileage != null && Number(form.initial_mileage) < lastMileage && <p className="flex items-center gap-1 text-xs font-medium text-tint-amber-foreground"><AlertTriangle className="h-3.5 w-3.5" />Entered mileage is below the last recorded value.</p>}
                  </div>
                  <div className="grid gap-2"><Label>Fuel level *</Label><div className="grid grid-cols-5 overflow-hidden rounded-xl border border-border bg-card" role="radiogroup" aria-label="Fuel level">{fuelLevels.map((level, index) => { const selectedIndex = fuelLevels.findIndex((item) => item.value === form.fuel_level); const active = index <= selectedIndex; return <button key={level.value} type="button" role="radio" aria-checked={form.fuel_level === level.value} aria-label={level.label} onClick={() => setField("fuel_level", level.value)} className={cn("h-11 border-r border-border text-sm font-bold last:border-r-0", active ? "bg-primary text-primary-foreground" : "bg-card text-muted-foreground")}>{level.short}</button>; })}</div><p className="text-xs font-medium text-muted-foreground">Selected: {fuelLevels.find((item) => item.value === form.fuel_level)?.label}</p></div>
                </div>
                <div className="mt-5 space-y-2">{charges.map((charge) => <div key={charge.id} className="grid grid-cols-[1fr_120px_44px] gap-2"><Select value={charge.label} onValueChange={(value) => setCharges((all) => all.map((item) => item.id === charge.id ? { ...item, label: value as ChargeLabel } : item))}><SelectTrigger className={inputClass}><SelectValue /></SelectTrigger><SelectContent>{chargeLabels.map((label) => <SelectItem key={label} value={label}>{label}</SelectItem>)}</SelectContent></Select><Input type="number" min={0} step="0.01" placeholder="AED" className={inputClass} value={charge.amount} onChange={(e) => setCharges((all) => all.map((item) => item.id === charge.id ? { ...item, amount: e.target.value } : item))} /><Button type="button" variant="outline" className={cn("h-11", lightOutlineClass)} onClick={() => setCharges((all) => all.filter((item) => item.id !== charge.id))}><X className="h-4 w-4" /></Button></div>)}<Button type="button" variant="outline" className="h-11 border-primary/30 !bg-primary/10 !text-primary hover:!bg-primary/15" onClick={() => setCharges((all) => [...all, { id: uuid(), label: "Delivery", amount: "" }])}><Plus className="mr-2 h-4 w-4" /> Add charge</Button></div>
                <div className="mt-5 grid gap-4 md:grid-cols-2"><div ref={(node) => { fieldRefs.current.deposit_amount = node; }} className="grid gap-2 rounded-xl border border-tint-amber-foreground/20 bg-tint-amber p-4"><Label className="font-bold text-tint-amber-foreground">Security deposit (AED)</Label><Input type="number" min={0} className={cn(inputClass, fieldErrors.deposit_amount && errorInputClass)} value={form.deposit_amount} onChange={(e) => setField("deposit_amount", e.target.value)} /><FieldError field="deposit_amount" /><p className="text-xs text-tint-amber-foreground">Deposit is separate from rental payments.</p></div><div className="grid gap-2"><Label>Notes</Label><Textarea className="min-h-28 !border-border !bg-input !text-foreground placeholder:!text-muted-foreground" placeholder="Optional internal notes" value={form.notes} onChange={(e) => setField("notes", e.target.value)} /></div></div>
              </section>

              <div className="rounded-xl border border-primary/30 bg-card p-4 shadow-lg md:sticky md:bottom-4 md:z-10"><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><div className="text-sm font-semibold text-muted-foreground">Contract total</div><div className="font-mono text-3xl font-bold">AED {grandTotal.toLocaleString()}</div><div className="text-xs text-muted-foreground">Rent AED {rentalTotal.toLocaleString()} + extras AED {extrasTotal.toLocaleString()}</div></div><Button className="h-12 bg-primary px-7 text-base font-bold text-primary-foreground hover:bg-primary/90" disabled={submitting} onClick={createContract}>{submitting ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : null}{submitting ? "Creating..." : "Create draft & continue"}</Button></div></div>
            </div>
          )}

          {step === 2 && <div className="space-y-4"><div><h2 className="text-2xl font-bold">Vehicle condition</h2><p className="mt-1 text-sm text-muted-foreground">Add pickup photos before the client signs. Maximum 10.</p></div><section className="rounded-xl border border-border bg-card p-4 md:p-5"><div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-5">{photos.map((photo) => <div key={photo.id} className="relative overflow-hidden rounded-xl border border-border bg-muted"><img src={photoPreviews[photo.id]} alt="Pickup inspection" className="aspect-square w-full object-cover" /><button type="button" aria-label="Delete photo" onClick={() => deletePhoto(photo)} className="absolute right-2 top-2 flex h-9 w-9 items-center justify-center rounded-full bg-card text-destructive shadow"><X className="h-4 w-4" /></button></div>)}</div>{photos.length === 0 && <div className="flex min-h-52 flex-col items-center justify-center rounded-xl border-2 border-dashed border-border bg-muted/20 text-center"><ImageIcon className="h-9 w-9 text-primary" /><p className="mt-3 font-semibold">No pickup photos yet</p><p className="text-sm text-muted-foreground">Take clear photos of all sides and existing damage.</p></div>}<input ref={cameraRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={(e) => { uploadPhotos(e.target.files); e.target.value = ""; }} /><input ref={galleryRef} type="file" accept="image/*" multiple className="hidden" onChange={(e) => { uploadPhotos(e.target.files); e.target.value = ""; }} /><div className="mt-5 grid gap-3 sm:grid-cols-2"><Button variant="outline" className="h-12 border-primary/30 !bg-primary/10 !text-primary hover:!bg-primary/15" disabled={uploading || photos.length >= 10} onClick={() => cameraRef.current?.click()}>{uploading ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <Camera className="mr-2 h-5 w-5" />}Take photo</Button><Button variant="outline" className={cn("h-12", lightOutlineClass)} disabled={uploading || photos.length >= 10} onClick={() => galleryRef.current?.click()}><ImageIcon className="mr-2 h-5 w-5" />Choose from gallery</Button></div></section><div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><Button variant="outline" className={cn("h-12", lightOutlineClass)} onClick={() => setStep(3)}>Skip photos for now</Button><Button className="h-12 bg-primary px-7 text-primary-foreground" onClick={() => setStep(3)}>Continue to review</Button></div></div>}

          {step === 3 && <div className="space-y-4"><div><h2 className="text-2xl font-bold">Review & sign</h2><p className="mt-1 text-sm text-muted-foreground">The draft becomes active after every required signature is saved.</p></div><section className="rounded-2xl border border-border bg-card p-4 shadow-sm md:p-5"><div className="grid grid-cols-2 gap-2.5 sm:gap-3"><Summary label="Client" value={selectedClient?.full_name || "—"} wide /><Summary label="Vehicle" value={selectedCar ? `${selectedCar.plate} — ${selectedCar.make} ${selectedCar.model}` : "—"} wide /><Summary label="Rental period" value={`${form.start_date} ${form.start_time} → ${form.end_date} ${form.end_time}`} wide /><Summary label="Rate" value={`${form.rate_type} · AED ${Number(form.rate_amount || 0).toLocaleString()}`} /><Summary label="Initial mileage" value={`${Number(form.initial_mileage || 0).toLocaleString()} ${form.mileage_unit}`} /><Summary label="Fuel" value={form.fuel_level} /><Summary label="Rental total" value={`AED ${rentalTotal.toLocaleString()}`} /><Summary label="Additional charges" value={`AED ${extrasTotal.toLocaleString()}`} /><Summary label="Deposit held separately" value={`AED ${Number(form.deposit_amount || 0).toLocaleString()}`} accent="amber" /><Summary label="Pickup photos" value={`${photos.length} photo${photos.length === 1 ? "" : "s"}`} /></div><div className="mt-3 rounded-xl bg-primary/10 p-3.5"><div className="text-xs font-semibold text-primary">Contract total</div><div className="font-mono text-2xl font-bold text-primary">AED {grandTotal.toLocaleString()}</div></div></section><div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-between"><Button variant="outline" className={cn("h-12", lightOutlineClass)} onClick={() => setStep(2)}>Back to photos</Button><Button className="h-12 bg-primary px-7 text-primary-foreground" onClick={() => setSignOpen(true)}>Open agreement & sign</Button></div><SignContractModal contractId={contractId} clientName={clientName} open={signOpen} onActivate={activateSignedContract} onComplete={finishSignedContract} /></div>}

          {step === 4 && <div className="mx-auto max-w-2xl py-12 text-center"><div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-tint-green text-tint-green-foreground"><Check className="h-10 w-10" /></div><h2 className="mt-6 text-3xl font-bold">Contract signed</h2><p className="mt-2 text-muted-foreground">The contract is active and the vehicle status has been updated.</p><div className="mt-8 grid gap-3 sm:grid-cols-2"><Button variant="outline" className={cn("h-12", lightOutlineClass)} onClick={() => navigate(`/contracts/${contractId}`)}>Open contract</Button><Button className="h-12 bg-primary text-primary-foreground" onClick={() => navigate("/contracts")}>Back to contracts</Button></div></div>}
        </div>
      </div>
    </DashboardLayout>
  );
...
function Summary({ label, value, accent, wide }: { label: string; value: string; accent?: "amber"; wide?: boolean }) {
  return <div className={cn("min-w-0 rounded-xl border p-3", wide && "col-span-2", accent === "amber" ? "border-tint-amber-foreground/20 bg-tint-amber" : "border-border bg-muted/20")}><div className={cn("text-[10px] font-bold uppercase tracking-wide", accent === "amber" ? "text-tint-amber-foreground" : "text-muted-foreground")}>{label}</div><div className="mt-1 break-words text-sm font-semibold leading-snug text-foreground">{value}</div></div>;
}
