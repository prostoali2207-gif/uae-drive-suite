        ...
        <div className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 backdrop-blur">
          <div className="mx-auto max-w-5xl px-4 py-4 md:px-8">
            <div className="flex items-center justify-between gap-3">
              <Button variant="ghost" className="h-11 px-2 text-slate-700 hover:bg-slate-100 hover:text-slate-950" onClick={() => navigate("/contracts")}>
                <ArrowLeft className="mr-2 h-4 w-4" /> Contracts
              </Button>
              <span className="text-sm font-bold text-blue-700">{step}/4</span>
            </div>
            <div className="mt-3 grid grid-cols-4 gap-2">
              {stepLabels.map((label, index) => {
                const number = index + 1;
                return <div key={label}><div className={cn("h-2 rounded-full", number <= step ? "bg-blue-600" : "bg-slate-200")} /><div className={cn("mt-1 hidden text-[11px] font-semibold md:block", number === step ? "text-blue-700" : "text-slate-500")}>{label}</div></div>;
              })}
            </div>
          </div>
        </div>
          ...
          {step === 1 && (
            <div className="space-y-5">
              ...
              <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:p-6">
                <h3 className="mb-4 text-sm font-bold uppercase tracking-wide text-blue-700">Client & vehicle</h3>
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="grid gap-2"><Label className="text-slate-800">Main client *</Label><Popover open={clientOpen} onOpenChange={setClientOpen}><PopoverTrigger asChild><Button variant="outline" className="h-11 justify-between border-slate-300 bg-white text-slate-950">{selectedClient?.full_name || "Select client"}<ChevronRight className="h-4 w-4 text-slate-400" /></Button></PopoverTrigger><PopoverContent className="w-[var(--radix-popover-trigger-width)] border-slate-200 bg-white p-0 text-slate-950"><Command className="bg-white"><CommandInput placeholder="Search client" value={clientSearch} onValueChange={setClientSearch} /><CommandList><CommandEmpty>No client found.</CommandEmpty><CommandGroup>{filteredClients.map((client) => <CommandItem key={client.id} value={client.id} onSelect={() => { setForm((p) => ({ ...p, client_id: client.id })); setAdditionalDriverIds((ids) => ids.filter((id) => id !== client.id)); setClientOpen(false); }}><Check className={cn("mr-2 h-4 w-4", form.client_id === client.id ? "opacity-100" : "opacity-0")} />{client.full_name}</CommandItem>)}</CommandGroup></CommandList></Command></PopoverContent></Popover></div>
                  <div className="grid gap-2"><Label className="text-slate-800">Vehicle *</Label><Popover open={carOpen} onOpenChange={setCarOpen}><PopoverTrigger asChild><Button variant="outline" className="h-11 justify-between border-slate-300 bg-white text-slate-950">{selectedCar ? `${selectedCar.plate} — ${selectedCar.make} ${selectedCar.model}` : "Select available vehicle"}<ChevronRight className="h-4 w-4 text-slate-400" /></Button></PopoverTrigger><PopoverContent className="w-[var(--radix-popover-trigger-width)] border-slate-200 bg-white p-0 text-slate-950"><Command className="bg-white"><CommandInput placeholder="Search plate or model" value={carSearch} onValueChange={setCarSearch} /><CommandList><CommandEmpty>No available vehicle found.</CommandEmpty><CommandGroup>{filteredCars.map((car) => <CommandItem key={car.id} value={car.id} onSelect={() => { prefillMileage(car.id); setCarOpen(false); }}><Check className={cn("mr-2 h-4 w-4", form.car_id === car.id ? "opacity-100" : "opacity-0")} />{car.plate} — {car.make} {car.model}</CommandItem>)}</CommandGroup></CommandList></Command></PopoverContent></Popover></div>
                </div>
              ...
              <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:p-6">
                <h3 className="mb-4 text-sm font-bold uppercase tracking-wide text-blue-700">Rental period & rate</h3>
              ...
              <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:p-6">
                <h3 className="mb-4 text-sm font-bold uppercase tracking-wide text-blue-700">Handover, extras & deposit</h3>
                <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="mt-5 space-y-2">{charges.map((charge) => <div key={charge.id} className="grid grid-cols-[1fr_120px_44px] gap-2"><Select value={charge.label} onValueChange={(value) => setCharges((all) => all.map((item) => item.id === charge.id ? { ...item, label: value as ChargeLabel } : item))}><SelectTrigger className={inputClass}><SelectValue /></SelectTrigger><SelectContent>{chargeLabels.map((label) => <SelectItem key={label} value={label}>{label}</SelectItem>)}</SelectContent></Select><Input type="number" min={0} step="0.01" placeholder="AED" className={cn(inputClass, "font-mono")} value={charge.amount} onChange={(e) => setCharges((all) => all.map((item) => item.id === charge.id ? { ...item, amount: e.target.value } : item))} /><Button type="button" variant="outline" className="h-11 border-slate-300 bg-white text-slate-700" onClick={() => setCharges((all) => all.filter((item) => item.id !== charge.id))}><X className="h-4 w-4" /></Button></div>)}<Button type="button" variant="outline" className="h-11 border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-100" onClick={() => setCharges((all) => [...all, { id: uuid(), label: "Delivery", amount: "" }])}><Plus className="mr-2 h-4 w-4" /> Add charge</Button></div>
