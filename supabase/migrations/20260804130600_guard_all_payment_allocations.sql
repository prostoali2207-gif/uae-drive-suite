...
begin
  ...
  if new.contract_id is null then
    raise exception 'Paid payment must belong to a contract';
  end if;
  ...
  loop
    ...
    if allocation.key like 'rental-%' then
      category := 'rental';
      begin item_id := substring(allocation.key from 8)::uuid;
      exception when others then raise exception 'Invalid rental allocation reference: %', allocation.key; end;

      if item_id is distinct from new.contract_id then
        raise exception 'Rental allocation belongs to another contract: %', item_id;
      end if;
      ...
      category := 'fees';
      begin item_id := substring(allocation.key from 5)::uuid;
      exception when others then raise exception 'Invalid fee allocation reference: %', allocation.key; end;
      ...
      category := 'fines';
      begin item_id := substring(allocation.key from 6)::uuid;
      exception when others then raise exception 'Invalid fine allocation reference: %', allocation.key; end;
      ...
      category := 'salik';
      begin item_id := substring(allocation.key from 7)::uuid;
      exception when others then raise exception 'Invalid Salik allocation reference: %', allocation.key; end;
      ...
      if abs(allocation.value::numeric - target_amount) > 0.009 then
        raise exception 'Salik must be paid in full for transaction %', item_id;
      end if;
  ...
  if abs(line_total - new.amount) > 0.009 then
    raise exception 'Payment amount must equal allocated line total';
  end if;
  ...
  if abs(coalesce((new.allocations ->> 'rental')::numeric, 0) - rental_total) > 0.009
    ...
    or abs(coalesce((new.allocations ->> 'salik')::numeric, 0) - salik_total) > 0.009 then
...
drop trigger if exists validate_payment_allocations_trigger on public.payments;
create trigger validate_payment_allocations_trigger
before insert or update of status, allocations, contract_id, amount
on public.payments
for each row
execute function public.validate_payment_allocations();
...
begin
  for line_key in
    ...
    where key like 'fine-%' or key like 'salik-%'
  loop
...
drop trigger if exists sync_charge_statuses_from_payments_trigger on public.payments;
create trigger sync_charge_statuses_from_payments_trigger
after insert or update of status, allocations, contract_id, amount or delete
on public.payments
for each row
execute function public.sync_charge_statuses_from_payments();