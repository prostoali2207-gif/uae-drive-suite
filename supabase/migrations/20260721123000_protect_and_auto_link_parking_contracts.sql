...
begin
  ...
  if new.contract_id is null then
    select count(*), min(q.contract_id), min(q.client_id)
...
drop trigger if exists trg_enforce_parking_contract_rental_period on public.parking_charges;
create trigger trg_enforce_parking_contract_rental_period
before insert or update of contract_id, client_id, car_id, parking_date
on public.parking_charges
for each row execute function public.enforce_parking_contract_rental_period();